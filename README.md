...
- **Vite 5** with hardened Cloudflare Worker proxy for server-side URL fetching
...
```
even-browser/
  vite-plugin.ts                 # Resolve aliases (even-dev compatible)
  src/
...
Phone UI (enter URL)
    ↓
Hardened Cloudflare Worker proxy fetches HTML server-side (text-only, size/timeout capped)
    ↓
